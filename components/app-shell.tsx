...
export function AppShell({ children, name }: { children: React.ReactNode; name: string }) {
  ...
  return <div className="min-h-screen bg-background"><div className="fixed inset-y-0 left-0 hidden w-64 border-r lg:block">{sidebar}</div>{open && <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm lg:hidden" onClick={() => setOpen(false)}><div className="h-full w-72 border-r" onClick={event => event.stopPropagation()}>{sidebar}</div></div>}<div className="lg:pl-64"><header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b bg-background/90 px-4 backdrop-blur-xl sm:px-6 lg:px-8"><Button variant="ghost" size="icon" className="lg:hidden" onClick={() => setOpen(true)} aria-label="Open menu"><Menu /></Button><p className="hidden font-mono text-[10px] uppercase tracking-[.22em] text-muted-foreground sm:block">Production workspace</p><div className="ml-auto intelligent-status"><BrainCircuit className="size-3.5" aria-hidden="true" /><span className="font-mono text-[10px] uppercase tracking-[.16em]">Intelligent studio</span><span className="intelligent-status-dot" /></div></header><div className="relative min-h-[calc(100vh-4rem)] overflow-hidden"><IntelligentGeometry variant="app" /><main className="relative z-10 mx-auto w-full max-w-[1440px] p-4 sm:p-6 lg:p-8">{children}</main></div></div></div>
}
